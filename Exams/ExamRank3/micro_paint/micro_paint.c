...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/02 15:22:28 by epfennig          #+#    #+#             */
/*   Updated: 2021/06/02 16:16:16 by epfennig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_in(t_data *d, float x, float y)
{
	if ((x < d->posx) || (x > d->posx + d->wrect) || (y < d->posy) || (y > d->posy + d->hrect))
		return (0);
	if (d->type == 'r' && ((x - d->posx < 1.00000) || (d->posx + d->wrect - x < 1.00000) || (y - d->posy < 1.00000) || (d->posy + d->hrect - y < 1.00000)))
		return (1);
	else if (d->type == 'R')
		return (2);
	return (0);
...
int		exec_boucle(t_data *d, FILE *file)
{
	...
	if (!(d->tab))
		return (-1);
	ret = fscanf(file, "%c %f %f %f %f %c\n", &d->type, &d->posx, &d->posy, &d->wrect, &d->hrect, &d->c);
	while (ret == 6)
	{
		if (check_data(d) == 1)
			exec_one(d);
